...
bool Renderer::Initialize(const float screen_width, const float screen_height)
{
	...
	m_shaders["link"] = LoadShader("Shaders/1_1_Model.vert", "Shaders/1_1_Model.frag");
	m_shaders["normal"] = LoadShader("Shaders/1_2_Model_Normal.vert", "Shaders/1_2_Model_Normal.frag", "Shaders/1_2_Model_Normal.geom");
...
void Renderer::DrawModels()
{
	...
	m_shaders["normal"]->SetActive();
	m_shaders["normal"]->SetMat4("projection", p);
	m_shaders["normal"]->SetMat4("view", v);
	m_shaders["normal"]->SetMat4("model", m);
	m_link->Draw(*m_shaders["normal"]);
}
...
{
	//m_link = new Model();
	cout << "Load Chracter! " << path.c_str() << endl;
...
unique_ptr<Shader> Renderer::LoadShader(const string& vert, const string& frag, const string& geom)
{
